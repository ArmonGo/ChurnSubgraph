...
"""
@inproceedings{DBLP:conf/icml/0002QZYZ0ZWHT24,
...
"""
# https://github.com/ZhiningLiu1998/BAT?tab=readme-ov-file
...
class BaseGraphAugmenter(ABC):
    ...
    @abstractmethod
    def augment(
        self, model: torch.nn.Module, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor
    ):
        """
        Perform graph augmentation.
...
class BatAugmenter(BaseGraphAugmenter):
    ...
    def buid_avg_edge_attr(self, edge_attr, new_edge_index):
        if edge_attr is not None:
            avg_edge_attr = edge_attr.mean(dim = 0). reshape(1, -1)
            return avg_edge_attr.repeat(new_edge_index.shape[1], 1)
    ...
    def augment(
        ...
        y_pred_proba = self.predict_proba(model,x, edge_index, edge_attr)
        ...
        if edge_attr is not None:
            edge_attr_aug = self.buid_avg_edge_attr(edge_attr, virtual_edge_index)
            edge_attr_aug = torch.concat([edge_attr, edge_attr_aug], axis=0)
        else:
            edge_attr_aug = None
        info = {
        ...
        return x_aug, edge_index_aug, edge_attr_aug, info
    ...
    def predict_proba(
        ...
        model.eval()
        with torch.no_grad():
            logits = model.forward(x, edge_index, edge_attr)
    ...
    def edge_sampling(
        ...
        try:
            ...
            print('edge sampling proba',edge_sampling_proba, edge_sampling_proba.shape )
