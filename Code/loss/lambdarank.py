# code: https://github.com/rjagerman/pytorchltr/blob/9f5cdee8d08cf6401c3e647ffd0fd673b0706442/pytorchltr/loss/pairwise_lambda.py
...
from loss.operations import batch_pairs as _batch_pairs
from loss.operations import rank_by_score as _rank_by_score
from loss.operations import prepare_lambdaloss_inputs as _prepare_lambdaloss_inputs
...
class LambdaLoss(_torch.nn.Module):
    ...
    #            n: _torch.LongTensor) -> _torch.FloatTensor:
        ...
        Args:
            ...
            n: A batch of per-query number of documents (for padding purposes).
        """
        # scores, relevance, n = _prepare_lambdaloss_inputs(y_pred, y_true, group_num)
        ...
        if loss_pairs is None:
            print('loss contains nan and stop traing here')
            return None
...
class LambdaNDCGLoss2(LambdaLoss):
    ...
    def _loss_per_doc_pair(self, score_pairs, rel_pairs, n):
        ...
        ## add threshold (for stable loss)
        gain_diffs[torch.abs(gain_diffs) < 1e-4] = 0.0 # > add threshold 
        ...
        delta = delta.clamp(min=1e-3) #> clip again to avoid inf 

        # Compute final loss
        exponent = delta[None, :, :] * _torch.abs(gain_diffs)
        
        # sigmoid = (1.0 / (1.0 + _torch.exp(-self.sigma * score_diffs)))
        # score_diffs = torch.nan_to_num(score_diffs, nan=0.0, posinf=50.0, neginf=-50.0).clamp(-50, 50) # > clamp for safe 
        sigmoid = torch.sigmoid(self.sigma * score_diffs)  #> numerical stable 

        safe_sigmoid =  sigmoid.clamp(min=1e-4, max=1.0 - 1e-4) # clamp for safe
        safe_exponent = exponent.clamp(min=0.0, max=5.0) # clamp for safe 

        # Consider using the above to make it safer

        # loss = _torch.log2(sigmoid ** exponent)
        loss = safe_exponent * _torch.log2(safe_sigmoid) # <- is the same
...
def _max_dcg(relevance: _torch.FloatTensor, n: _torch.LongTensor,
    ...
    ranking = _rank_by_score(relevance.float(), n)
