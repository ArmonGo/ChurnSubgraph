...
def prepare_lambdaloss_inputs(y_pred, y_true, group_num):
    """
    Convert flat tensors and group sizes to padded batches suitable for Lambda loss.

    Args:
        y_pred (Tensor): shape [N], predicted scores for all documents.
        y_true (Tensor): shape [N], ground truth relevance scores.
        group_num (List[int]): number of documents per query.

    Returns:
        padded_scores: Tensor [batch_size, max_group_len]
        padded_relevance: Tensor [batch_size, max_group_len]
        n: Tensor [batch_size], original group sizes before padding
    """
    scores_list = []
    relevance_list = []

    start = 0
    for n_docs in group_num:
        end = start + n_docs
        scores_list.append(y_pred[start:end])
        relevance_list.append(y_true[start:end])
        start = end

    padded_scores = pad_sequence(scores_list, batch_first=True, padding_value=0.0)
    padded_relevance = pad_sequence(relevance_list, batch_first=True, padding_value=0)

    n = torch.tensor(group_num, dtype=torch.long)

    return padded_scores, padded_relevance, n
